...
static void producerPublish(const uint16_t msgId, const uint8_t *payload,
                            uint8_t dlc);
static void handle_momentary_event(const uint32_t now);
...
static void canHealthLog(const canHealth_t *h) {
  ESP_LOGI("CANHEALTH",
           ...
           h->tx_error_count, h->rx_error_count, h->bus_error_count,
           h->arb_lost_count, h->rx_missed_count, h->rx_overrun_count,
           h->tx_failed_count, h->controller_state, h->bus_off_events,
           h->error_passive_events, h->recovery_resets, h->rx_queue_full_events,
  ...
  for (int i = 0; i < MAX_SUB_MODULES; i++) {
    ...
    switch (sub->personalityId) {
      ...
      case PERS_GPIO_OUTPUT:
        {
          value = digitalRead(myPin); /**< Capture digital value */
          break;
        }
        /* Analog ADC input */
      ...
      default:
        {
          ESP_LOGW(TAG, "[PRODUCER] Ignoring unknown personality %u",
                   sub->personalityId);
    ...
    rt->valueU32 = value;          /* update value */
    rt->last_change_ms = millis(); /* update timestamp */
  } /* end of for loop */
}
...
static void producerPublishEvent(producer_event_t *evt) {
  ...
  if (!sub) {
    ESP_LOGE(TAG, "[PRODUCER] Invalid or inactive submodule at index %u",
             subIdx);
    return;
  }
  ...
  if (dlc < CAN_DATAMSG_MIN_DLC)
    dlc = CAN_DATAMSG_MIN_DLC; /**< enforce minimum DLC for producer messages
                                  6-bytes */
  if (dlc > CAN_MAX_DLC)
  ...
  ESP_LOGV(TAG, "[PRODUCER] Tx: sub %u msg 0x%03X dlc %u val %u", subIdx,
           p->dataMsgId, dlc, valueU32);
}
...
static void internalSubmoduleTick(uint32_t now) {
  ...
  for (uint8_t i = 0; i < n; i++) {
    ...
    runTime_t *rt = nodeGetRuntime(list[i]);

    if (!sub || !rt)
      continue;

    ESP_LOGV(TAG, "[PRODUCER] Checking internal event for sub %u interval %u",
             list[i], sub->producer_period_ms);

    if (now - rt->last_published_value >= sub->producer_period_ms) {
      enqueueInputCmd(INPUT_CMD_INTERNAL_EVENT, list[i], now);
      rt->last_published_value = now;

      ESP_LOGV(TAG,
               "[PRODUCER] Enqueueing internal event for sub %u interval %u",
               list[i], sub->producer_period_ms);
    }
  }
}
...
static void managePeriodicMessages() {
  ...
  /* Repeat any momentary events */
  handle_momentary_event(currentMillis);
 ...
 * conditions are true, the ISR handler is attached to the corresponding GPIO
 * pin.
 *
 * @note This function is called once during initialization and should not be
 * called again.
 */
void attachGpioIsrInit(void) {
  for (int i = 0; i < MAX_SUB_MODULES; i++) {
    ...
    if (sub->personalityIndex != i) {
      ESP_LOGE("NODE", "Invalid personalityIndex: sub %d has %d", i,
               sub->personalityIndex);
      continue;
    }
...
static void handleInputCommand(inputCommandMsg_t *cmd) {
  ...
  switch (cmd->type) {
    ...
    case INPUT_CMD_GPIO_EVENT:
      {
        producer_event_t evt = producerHandleGpioEvent(subIdx, cmd->timestamp);

        if (evt.error) {
          ESP_LOGW(TAG, "[PRODUCER] GPIO event handler returned an error, "
                        "aborting producer processing.");
          break;
        } else if (evt.ready) {
          producerPublishEvent(&evt);
        } else { /* log a debug message if the event was processed but not ready
                    to publish */
          ESP_LOGD(TAG, "[PRODUCER] GPIO event ignored: not ready (sub=%u)",
                   subIdx);
        }
        break;
      }
    ...
    default: ESP_LOGW(TAG, "Unknown input event: %d", cmd->type); break;
  }
}

static void handle_momentary_event(const uint32_t now) {
  /** Momentary-mode repetition */
  for (uint8_t i = 0; i < MAX_SUB_MODULES; i++) {
    if (!nodeIsActiveSubmodule(i))
      continue;
    ...
    uint32_t v = rt->valueU32;

    // ACTIVE_VALUE → repeat every producer_period_ms
    if (v == MOMENTARY_ACTIVE_VALUE) {
      if (now - rt->last_published_ms >= sub->producer_period_ms) {
        ...
        evt.sub_idx = i;
        evt.timestamp = now;
        evt.value = MOMENTARY_ACTIVE_VALUE;

        rt->last_published_ms = now;
        rt->publish_count = 0; /* reset counter for the release message */

        producerPublishEvent(&evt);
      }
    }
    // RELEASE_VALUE → publish once
    else if (v == MOMENTARY_RELEASE_VALUE) {
      if (rt->publish_count < MOMENTARY_RELEASE_REPEAT) {
        if (now - rt->last_published_ms >= sub->producer_period_ms) {
          producer_event_t evt = {0};
          ...
          evt.sub_idx = i;
          evt.timestamp = now;
          evt.value = MOMENTARY_RELEASE_VALUE;

          rt->publish_count++;
          rt->last_published_value = MOMENTARY_RELEASE_VALUE;
          rt->last_published_ms = now;
...
void startTaskProducer(void) {
  xTaskCreate(TaskProducer, "Task Producer", TASK_PRODUCER_STACK_SIZE, NULL,
              tskLowPriority, &xProducerHandle);
}
